...
import { TradeParams } from "../../core/types/Trading.ts";
import { JupiterService } from "../dex/jupiter/jupiter.ts";
import { Logger } from "jsr:@deno-library/logger";
import { swapJupyter } from "../dex/jupiter/utils.ts";
...
export class TradeExecutor {
  ...
  async executeSell(params: TradeParams) {
    // try {
    ...
    //   throw new Error(`Sell execution failed: ${(error as Error).message}`);
    // }
  }
  ...
  private async executeTransaction(
    ...
    try {
      ...
      if (this.shouldRetry(error) && attempt < this.MAX_RETRIES) {
        ...
        this.logger.info(
          `Retrying transaction in ${delayMs}ms (attempt ${attempt + 1} of ${
            this.MAX_RETRIES
          })...`
        );
