...
export class SolanaWallet {
  ...
  async getTokenDecimals(mint: PublicKey): Promise<number> {
    try {
      ...
      // Type-safe check for parsed account data
      if (tokenInfo.value?.data) {
        const accountData = tokenInfo.value.data;

        // Check if it's parsed account data
        if ("parsed" in accountData) {
          return accountData.parsed?.info?.decimals ?? 9;
        }
      }
      ...
      this.logger.error(`Error fetching token decimals for ${mint}:`, error);
      return 9;
    }
  }
  ...
  async getTokenBalance(
    ...
    try {
      ...
      this.logger.error(
        `Error fetching token balance for ${mintPublicKey}:`,
        error
      );
      return { amount: 0n };
    }
