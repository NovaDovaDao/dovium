...
export class BuyTokenTransaction extends BaseTransaction {
  override logger = new DoviumLogger(BuyTokenTransaction.name);
    ...
    try {
      ...
      while (retryCount < config.swap.token_not_tradable_400_error_retries) {
        try {
          ...
          this.logger.log(
            `Attempting to get quote (attempt ${retryCount + 1})`
          );
          this.logger.log(
            `Input: ${solMint}, Output: ${tokenMint}, Amount: ${amount}`
          );
    ...
    while (retryCount < config.tx.fetch_tx_max_retries) {
      try {
        ...
        const solTokenAccount =
          accountOne === config.liquidity_pool.wsol_pc_mint
            ? accountOne
            : accountTwo;
